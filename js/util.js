 ...
 *       仅支持#id   .class1 class2 li  三种简单类型
 * @param  {[Element]} context  [查找的根元素]
 * @return {[List|Element]}          [#id直接返回元素，其他选择器返回结果集]
 *
 *
 * 支持到IE8+则用querySelector代替
 */
function $(selector,context){
  if(selector.charAt(0) === '#'){
    ...
    if(context.getElementsByClassName){
      return context.getElementsByClassName(clazz);
    }else{
      var ret = [],
          arr = clazz.trim().split(/\s+/),
          childList = context.getElementsByTagName('*'),
          i,j;
      for(i=0;i<childList.length;i++){
        var classes = childList[i].className.trim().split(/\s+/),
          flag = !0;
...
/* js工具 */
var util = {
   //addEvent removeEvent 元素绑定/解绑事件兼容,兼容性判断只做一次,提高效率
   /**
    * [description]
    * @param  a  绑定或解绑事件的元素
    * @param  b  事件类型
    * @param  c  事件
    * @param  d  是否捕获，默认false，低版本IE也不支持捕获
